export default class TouchController{
  ...
  #start(evt){const target=evt.target.closest('.item-node');if(!target)return;const box=target.closest('.box');this.#active={node:target,originBox:box?.dataset.boxId,itemId:target.dataset.itemId};target.setPointerCapture?.(evt.pointerId);target.classList.add('item-floating');}
  #move(evt){if(!this.#active)return;const {node}=this.#active;node.style.transform=`translate(${evt.clientX-32}px, ${evt.clientY-32}px)`;}
