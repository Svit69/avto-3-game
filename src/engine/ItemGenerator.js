...
export default class ItemGenerator{
  ...
  buildPack(triplesNeeded,fillerScale,fillLimit){const items=[];const addTriple=(asset)=>{for(let i=0;i<3;i+=1)items.push(this.#spawn(asset));};for(let i=0;i<triplesNeeded;i+=1)addTriple(this.#pickAsset());while(items.length<Math.min(fillLimit,triplesNeeded*fillerScale*3))addTriple(this.#pickAsset());while(items.length<fillLimit)items.push(this.#spawn(this.#pickAsset()));return this.#shuffle(items);}
