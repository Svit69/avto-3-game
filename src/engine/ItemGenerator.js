...
export default class ItemGenerator{
  ...
  buildPack(triplesNeeded,fillerScale,maxItems){const items=[];const addTriple=(asset)=>{for(let i=0;i<3;i+=1)items.push(this.#spawn(asset));};for(let i=0;i<triplesNeeded;i+=1)addTriple(this.#pickAsset());while(items.length<Math.min(maxItems,triplesNeeded*fillerScale*3))addTriple(this.#pickAsset());while(items.length<maxItems)items.push(this.#spawn(this.#pickAsset()));return this.#shuffle(items);}
