...
export default class GameBoard{
  ...
  seed(level){const fillPerBox=Math.max(1,this.#boxCapacity-1);const pack=this.#generator.buildPack(level.triples,level.filler,this.#boxes.size*fillPerBox,this.#currentCounts());[...this.#boxes.values()].forEach((box)=>box.resetBox());let cursor=0;this.#boxes.forEach((box)=>{for(let i=0;i<fillPerBox;i+=1){const item=pack[cursor];if(!item)return;box.pushItem(item.createNode(),item);cursor+=1;}});}
  ...
  #seedSingle(box){const fill=Math.max(1,this.#boxCapacity-1);this.#generator.buildPack(0,1,fill,this.#currentCounts()).forEach((item)=>box.pushItem(item.createNode(),item));}
