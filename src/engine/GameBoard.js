...
export default class GameBoard{
  ...
  seed(level,rng,planQueue){this.#level=level;this.#rng=rng;this.#allowedTypes=level.types;this.#planQueue=planQueue;[...this.#boxes.values()].forEach((box)=>box.resetBox());const counts=this.#currentCounts();this.#boxes.forEach((box)=>{const fill=this.#fillCount(this.#level);this.#generator.buildPack(fill,counts,this.#allowedTypes,this.#rng,this.#planQueue).forEach((item)=>box.pushItem(item.createNode(),item));});}
  ...
  #seedSingle(box){const counts=this.#currentCounts();const fill=this.#fillCount(this.#level||{fillMin:1,fillMax:this.#boxCapacity-1});this.#generator.buildPack(fill,counts,this.#allowedTypes,this.#rng,this.#planQueue).forEach((item)=>box.pushItem(item.createNode(),item));}
