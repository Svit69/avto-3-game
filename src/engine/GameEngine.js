...
export default class GameEngine{
  ...
  #launch(level){if(!level)return;this.#rng=new SeededRandom(level.seed||1);this.#planQueue=this.#buildPlan(level);this.#triplesLeft=level.triples;this.#secondsLeft=level.seconds;this.#hud.setLevel(level.name);this.#hud.setTriplesLeft(this.#triplesLeft);this.#hud.setTimer(this.#secondsLeft);this.#board.seed(level,this.#rng,this.#planQueue);this.#checkState(level);clearInterval(this.#timerRef);this.#timerRef=setInterval(()=>this.#tick(),1000);}
  ...
  #onTriple(box){this.#triplesLeft-=1;this.#hud.setTriplesLeft(this.#triplesLeft);this.#hud.flash('\u041a\u043e\u0440\u043e\u0431\u043a\u0430 \u0437\u0430\u043a\u0440\u044b\u0442\u0430!');box.markClosed();setTimeout(()=>{this.#board.replaceBox(box);this.#checkState(this.#levels.current());if(this.#triplesLeft<=0)this.#win();},600);}
  ...
  #checkState(level){const report=this.#validator.validateState(level,this.#board.snapshot(),this.#planQueue,3);if(!report.ok){this.#hud.flash(report.message);console.warn(report.fix);}}
}
